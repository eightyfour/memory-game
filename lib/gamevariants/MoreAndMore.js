...
function MoreAndMore(player, cards, util) {
    ...
    // ignore hideCards if game ends
    clientListener.hideCards = function () {
        if (!gameStatus.gameOver) {
            player.client.hideCards.apply(null, [].slice.call(arguments));
        }
    };
    ...
    serverListener.takeCard = function (sId, cardPosition, doneCb) {
        ...
        if (gameStatus.doubleSelected >= gameRules.maxDoubleSelected) {
            ...
            // show all cards
            singlePlayerGame.game.showAll(sId, doneCb);
