...
function createSingleGame(player,gameConfig){
    var cardSet;
    if(gameConfig.numberOfSymbols && SINGLE_CARDS.length > gameConfig.numberOfSymbols){
        cardSet = shuffle(SINGLE_CARDS.splice(0,gameConfig.numberOfSymbols));
    } else {
        cardSet = SINGLE_CARDS;
    }
    var newCards = createCardObjects(shuffle(cardSet.concat(cardSet)));
        ...
        startNewGame : function (userId, gameConfig, callback) {
            ...
            BLOCK.filter(function(fc){
                ...
                switch(gametype){
                    ...
                    default : conf = createSingleGame(player,gameConfig);
                }
